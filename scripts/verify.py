...
def watchOnCollectionStatus():
    property_value = ""
    print(f"🔍 Watching for '{COLLECTION_PROPERTY_NAME}' property changes...\n")

    with subprocess.Popen(COLLECTION_MONITOR_CMD, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True) as proc:
        try:
            capture = False
            buffer = []
                ...
                if "signal" in line and "PropertiesChanged" in line:
                    ...
                    if line == "]":  # End of the array
                        ...
                        if COLLECTION_PROPERTY_NAME in full_signal:
                            match = re.search(r'variant\s+string\s+"([^"]+)"', full_signal)
...
def update_eeprom_status():
    ...
    for item in verify_report:
        if item["type"] == "valid_eeprom":
            value = get_dbus_property("xyz.openbmc_project.Inventory.Manager", item["inventory_path"], "com.ibm.VPD.Collection", "CollectionStatus")
            ...
            for pel in pel_data:
                if "User Data 1" in pel and "DESCRIPTION" in pel["User Data 1"] and item["eeprom_path"] in pel["User Data 1"]["DESCRIPTION"]:
