...
int VpdTool::dumpObject(const std::string& i_objectPath,
                        ...
                        nlohmann::json& io_resultJson) const noexcept
{
    ...
    try
    {
        ...
        if (!i_sysConfigJson.contains("frus"))
        {
            std::cerr << "System Config JSON is not valid" << std::endl;
            return l_rc;
        }

        std::string l_eepromPath{};
        // get the EEPROM path for this object path
        l_eepromPath = jsonUtility::getFruPathFromJson(i_sysConfigJson,
                                                       i_objectPath);
    ...
    {
        // TODO: Enable logging when verbose is enabled.
        /*std::cerr << "Dump Object failed for FRU " << i_objectPath <<
